...
class Etcpage_modify_mento : AppCompatActivity()  {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        profile_camera_image.setOnClickListener {
            ...
            popUpMenu.setOnMenuItemClickListener { menuItem ->
                ...
                if (id == 0) {
                    Toast.makeText(this@Etcpage_modify_mento, "옵션1 클릭", Toast.LENGTH_SHORT).show()
                    pickGallery()
                } else if (id == 1) {
                    Toast.makeText(this@Etcpage_modify_mento, "옵션2 클릭", Toast.LENGTH_SHORT).show()
                    changeToBasic()
                } else if (id == 2) {
                    Toast.makeText(this@Etcpage_modify_mento, "옵션3 클릭", Toast.LENGTH_SHORT).show()
                    captureCamera()
                }
                false
            }
    ...
    fun pickGallery() {
        /* 갤러리 권한 부여 */
        var permission= ContextCompat.checkSelfPermission(this@Etcpage_modify_mento, Manifest.permission.READ_EXTERNAL_STORAGE)
        if (permission == PackageManager.PERMISSION_DENIED) {
            // 권한이 없을 경우
            if (ActivityCompat.shouldShowRequestPermissionRationale(this@Etcpage_modify_mento, Manifest.permission.READ_EXTERNAL_STORAGE)) {
                // 사용자가 이전에 권한을 실수로 취소시킨 경우
                Toast.makeText(this@Etcpage_modify_mento, "프로필 사진을 설정하기 위해서는 갤러리 권한이 요구됩니다.", Toast.LENGTH_SHORT).show()
                // 권한 재요청
            ...
            // 권한이 있을 경우
            Toast.makeText(this@Etcpage_modify_mento, "갤러리 권한 이미 소지, 갤러리 실행", Toast.LENGTH_SHORT).show()
    ...
    fun captureCamera() {
        /* 카메라 권한 부여 */
        var permission= ContextCompat.checkSelfPermission(this@Etcpage_modify_mento, Manifest.permission.CAMERA)
        if (permission == PackageManager.PERMISSION_DENIED) {
            // 권한이 없을 경우
            if (ActivityCompat.shouldShowRequestPermissionRationale(this@Etcpage_modify_mento, Manifest.permission.CAMERA)) {
                // 사용자가 이전에 권한을 실수로 취소시킨 경우
                Toast.makeText(this@Etcpage_modify_mento, "프로필 사진을 설정하기 위해서는 카메라 권한이 요구됩니다.", Toast.LENGTH_SHORT).show()
                // 권한 재요청
            ...
            // 권한이 있을 경우
            Toast.makeText(this@Etcpage_modify_mento, "카메라 권한 이미 소지, 카메라 실행", Toast.LENGTH_SHORT).show()
    ...
    override fun onRequestPermissionsResult(requestCode: Int,
        ...
        when (requestCode) {
            ...
            REQUEST_READ_EXTERMAL_STORAGE -> {
                Toast.makeText(this@Etcpage_modify_mento, "grantResults", Toast.LENGTH_SHORT).show()
                if ((grantResults.isNotEmpty() &&
                            grantResults[0] == PackageManager.PERMISSION_GRANTED)) {
                    // 사용자가 갤러리 권한 동의
                    dispatchPickGalleryIntent()
                ...
                Toast.makeText(this@Etcpage_modify_mento, "권한 예외 처리", Toast.LENGTH_SHORT).show()
            }
        }
    }
...
//        }
        Intent(MediaStore.ACTION_IMAGE_CAPTURE).also { takePictureIntent ->
            ...
            takePictureIntent.resolveActivity(this@Etcpage_modify_mento.packageManager)?.also {
    ...
    private fun createImageFile(): File {
        ...
        val storageDir: File = this@Etcpage_modify_mento.getExternalFilesDir(Environment.DIRECTORY_PICTURES)
    ...
    private fun galleryAddPic() {
        Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE).also { mediaScanIntent ->
            ...
            this@Etcpage_modify_mento.sendBroadcast(mediaScanIntent)
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        if (requestCode == REQUEST_CAMERA && resultCode == RESULT_OK) {
            ...
            Toast.makeText(this@Etcpage_modify_mento, "취소 되었습니다.", Toast.LENGTH_SHORT).show()
            /* 임시 파일 삭제 해야함(일단 보류) */
//            if (tempFile != null) {
//                if (tempFile.exists()) {
//                    if (tempFile.delete()) {
